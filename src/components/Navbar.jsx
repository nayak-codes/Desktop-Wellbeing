...
export default function Navbar({ onOpenSearch }) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#090d16]/90 backdrop-blur-md border-b border-white/10">
      <div className="container mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <a href="#" className="flex items-center gap-2.5 text-decoration-none group">
          <img 
            ...
            className="w-8 h-8 rounded-lg shadow-md shadow-blue-500/20 group-hover:scale-105 transition-transform object-cover" 
          />
          <div className="flex items-center gap-1.5">
            ...
            <span className="px-1.5 py-0.5 text-[10px] font-semibold bg-blue-500/15 text-blue-400 border border-blue-500/20 rounded">
              Windows
            </span>
          </div>
        </a>
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          <a href="#demo" className="text-sm font-medium text-slate-300 hover:text-white transition-colors flex items-center gap-1.5">
            <span>Live Demo</span>
          </a>
          <a href="#how-it-works" className="text-sm font-medium text-slate-300 hover:text-white transition-colors">
            How It Works
          </a>
          <a href="#download" className="text-sm font-medium text-slate-300 hover:text-white transition-colors">
            Download
          </a>
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="hidden sm:flex items-center gap-2 px-2.5 py-1.5 bg-slate-900 border border-white/10 hover:border-slate-700 rounded-lg text-xs text-slate-400 hover:text-slate-200 transition-colors cursor-pointer"
            title="Search (Ctrl+K)"
          >
            <Search className="w-3.5 h-3.5 text-slate-400" />
            <span>Search</span>
            <kbd className="px-1.5 py-0.2 bg-white/5 border border-white/10 rounded text-[10px] font-mono text-slate-400">⌘K</kbd>
          </button>
      ...
      {/* Mobile Menu Drawer */}
      {mobileOpen && (
        <div className="md:hidden bg-[#090d16] border-b border-white/10 px-6 py-4 flex flex-col gap-3">
